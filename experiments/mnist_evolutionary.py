"""
Evolutionary search experiment on MNIST.
"""
...
def main():
    # Load config
    config_path = Path(__file__).parent.parent / 'configs' / 'mnist_evolutionary.yaml'
    with open(config_path, 'r') as f:
        config = yaml.safe_load(f)

    print("=" * 60)
    print("Evolutionary Search on MNIST")
    print("=" * 60)
    ...
    results_data = {
        ...
        'n_generations': config['n_generations'],
        'train_budget': config['train_budget'],
        'results': population,  # Full population
        'history': search.history,  # Generation-by-generation stats
    ...
    # Trajectory plot
    trajectory_plot = plot_dir / 'mnist_evolutionary_trajectory.png'
    viz.plot_search_trajectory(str(results_file), str(trajectory_plot))

    # Diversity plot
    diversity_plot = plot_dir / 'mnist_evolutionary_diversity.png'
